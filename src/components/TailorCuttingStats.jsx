...
function TailorCuttingStats({ groupedData, employeeScans, recentUpdates, productDetails }) {
        ...
        >
          ...
          <td className="px-6 py-4 whitespace-nowrap relative z-10">
            <div className="flex items-center gap-3">
              <div
                className={`w-45 h-45 overflow-hidden rounded-full flex items-center justify-center ${
                  isTopThree
            ...
            <div className="flex items-center gap-2">
              <span
                className={`text-8xl font-bold ${
                  isUpdated
                    ? 'text-green-700 bg-green-100 px-3 py-2 rounded-lg animate-pulse'
                    : isTopThree
                      ? 'text-orange-800'
                      : 'text-gray-900'
                }`}
          ...
          {/* Scan Count Column */}
          <td className="px-6 py-4 whitespace-nowrap relative z-10">
            <div className="flex gap-10 items-center justify-left">
              <span
                className={`
                  inline-flex items-center justify-center w-12 h-12 rounded-full text-8xl font-bold
                `}
              >
                {item.orderCount}
              </span>
              {item.location?.toLowerCase() === 'cutting master' && item.liningCount > 0 && (
                <span
                  className={`
                  inline-flex ml-10 items-center justify-center w-12 h-12 rounded-full text-6xl text-green-500 font-bold
                `}
                >
                  + {item.liningCount}
                </span>
  ...
  const Table = React.memo(({ title, data, icon, iconColor }) => {
    ...
    return (
      <div className="bg-white rounded-2xl border border-gray-200 shadow-lg relative">
        {/* Scroll Controls */}
        {hasEnoughData && (
        ...
        <div className="absolute top-4 right-4 flex items-center gap-2">
          ...
          ></div>
          <span className="text-xs text-gray-600">{isPaused ? 'Paused' : 'Auto-scrolling'}</span>
        </div>
        ...
        <div className="overflow-x-auto" ref={tableContainerRef}>
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-6 py-4 text-left text-6xl font-bold text-gray-600 uppercase tracking-wider">
                  Name
                </th>
                <th className="px-6 py-4 text-left text-6xl font-bold text-gray-600 uppercase tracking-wider">
                  Style
